...
class ValidatedDataFrame(BaseModel):
    ...
    def validate_df(cls, values):
        ...
        dict_schema = {
            ...
            "duedate_year": pa.Column("int32", checks=pa.Check.greater_than_or_equal_to(2020), coerce=True),
            "duedate_month": pa.Column("int32", checks=pa.Check.between(1, 12), coerce=True),
            "duedate_day": pa.Column("int32", checks=pa.Check.between(1, 31), coerce=True),
            "duedate_weekday": pa.Column("int32", checks=pa.Check.between(0, 6), coerce=True),
            "duedate_weekend": pa.Column("int32", checks=pa.Check.isin([0, 1]), coerce=True),
        }
