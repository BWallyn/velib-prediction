...
def test_get_weekend_basic(sample_dataframe_weekend):
    df_with_weekend = get_weekend(
        sample_dataframe_weekend.copy(), "date"
    )  # Important: copy the DataFrame
    expected_weekend = np.array([0, 0, 0, 0, 0, 1, 1, 0])
    np.testing.assert_array_equal(
        df_with_weekend["date_weekend"].values, expected_weekend
    )
...
def test_get_weekend_different_date_column_name(sample_dataframe_weekend):
    ...
    expected_weekend = np.array([0, 0, 0, 0, 0, 1, 1, 0])
    np.testing.assert_array_equal(
        df_with_weekend["my_date_weekend"].values, expected_weekend
    )
...
def test_extract_date_features(sample_dataframe_date):
    ...
    assert (
        df_with_features["date_str_weekday"].iloc[2] == 1
    )  # Tuesday (assuming Tuesday is weekday 1)
